...
> **Version**: 1.5.0 (Weather Integration)  
> **Date**: January 2026
...
A1 integrates with the **Open-Meteo** free weather API for real-time weather data. No API key required!
...
| `should_i_carry_umbrella(location)` | Rain prediction | "Should I carry umbrella?" |

#### API Details
- **Endpoint**: `api.open-meteo.com`
- **Geocoding**: Automatic location resolution
- **Data**: Temperature, humidity, wind, precipitation, weather codes
- **Forecast**: Up to 16 days ahead

#### Routing
The router detects weather-related keywords (`weather`, `rain`, `forecast`, `umbrella`, etc.) and extracts the location using regex patterns. Three intents are supported:
- `weather` → Current conditions
- `weather_forecast` → Multi-day forecast  
- `weather_rain` → Umbrella recommendation

### 🐧 Complete System Control (Arch Linux)
**File**: `skills/arch.py`

A1 provides **40+ voice commands** for complete control of your Arch Linux system.

#### Feature Categories

| Category | Functions | Backend Tools |
| :--- | :--- | :--- |
| **Power** | shutdown, reboot, suspend, lock | `systemctl`, hyprlock |
| **Audio** | volume up/down, mute | `pactl`, `wpctl` |
| **Media** | play/pause, next, previous, stop | `playerctl` (MPRIS) |
| **Display** | brightness up/down, night mode | `brightnessctl`, `gammastep` |
| **Network** | WiFi on/off/status | `nmcli` |
| **Bluetooth** | on/off/status | `bluetoothctl` |
| **Screenshot** | full screen, area select | `grim`, `slurp`, `scrot` |
| **Cleanup** | cache, logs, trash, orphans | `paccache`, `journalctl` |
| **Notifications** | do not disturb on/off | `dunstctl`, `makoctl` |
| **Clipboard** | clear, read | `wl-copy`, `xclip` |
| **Process** | kill, force kill | `pkill` |
| **Files** | file manager, downloads, documents | `nautilus`, `thunar`, etc. |
| **Timer** | set timer X minutes | `notify-send` |
| **Power Profiles** | performance, balanced, power-saver | `powerprofilesctl` |
| **Packages** | install, uninstall, update | `yay`, `pacman` |
| **System Info** | status, uptime, time | `psutil` |

#### Required Dependencies
```bash
yay -S brightnessctl networkmanager bluez bluez-utils gammastep \
       playerctl dunst grim slurp pacman-contrib pipewire-pulse
```
...
| :--- | :--- | :--- |
| **v1.5.0** | Jan 2026 | **Complete System Control** (40+ commands): Weather, Bluetooth, Media, Screenshots, Night Mode, Cleanup, DND, Timer, Power Profiles |
...
| **v1.0.0** | Jan 2026 | ReAct Loop, Basic System Control |
