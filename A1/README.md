...
| **Power** | "Shutdown", "Reboot", "Suspend", "Lock screen" | System power management. |
| **Audio** | "Volume up", "Volume down", "Mute" | PulseAudio/PipeWire control. |
| **Media** | "Play/Pause", "Next song", "Stop music" | MPRIS (Spotify, VLC, etc). |
| **Brightness** | "Brightness up", "Night mode on" | Display controls. |
| **Network** | "WiFi on", "Bluetooth off", "WiFi status" | Network management. |
| **Screenshot** | "Take a screenshot", "Screenshot area" | Saves to ~/Pictures. |
| **System** | "System status", "Uptime", "What time is it" | System info. |
| **Cleanup** | "Empty trash", "Clean cache", "Clear logs" | Maintenance. |
| **Files** | "Open downloads", "Open file manager" | Quick navigation. |
| **Timer** | "Set timer for 5 minutes" | Desktop notification. |
| **Focus** | "Do not disturb", "DND off" | Notification control. |
| **Packages** | "Install docker", "Update system" | Arch Linux (`yay`). |
| **Vision** | "Look at the screen", "Analyze this" | Screenshot → Gemini. |
| **Weather** | "Weather in Chennai", "5-day forecast" | Open-Meteo data. |
| **Chat** | "Tell me a joke", "Write code" | Llama 3.1. |
