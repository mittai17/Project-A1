...
- **`listen.py` (Deprecated by `adaptive_asr.py`)**
  - **Purpose**: High-accuracy command transcription.
  - **Key Function**: `listen_for_command(model, timeout)`
  - **Note**: This module is now replaced by the Whisper-based `adaptive_asr.py` in production but kept for fallback.

- **`adaptive_asr.py` (New in v1.1)**
  - **Purpose**: Speaker-aware transcription.
  - **Key Classes**: `AdaptiveEar`
  - **Logic**: 
    - Uses **OpenAI Whisper** for state-of-the-art transcription.
    - Uses **System Native Audio** (SoundDevice).
    - Verifies speaker using stored `user_profile.npy` (generated by `voice_enroll.py`).
    - Biases the transcription model with specific context if the user is verified.

- **`speaker_embed.py`**
  - **Purpose**: Voice Biometrics.
  - **Logic**: Uses `speechbrain/spkrec-ecapa-voxceleb` to convert audio chunks into 192-dimensional vectors (Embeddings). These are comparable using Cosine Similarity.
